...
def check_imports():
    ...
    try:
        ...
        print(f"⚠ MediaPipe import failed (fallback keypoints will be used): {e}")

    try:
        import rembg
        print(f"✓ rembg")
    except ImportError as e:
        print(f"⚠ rembg import failed (GrabCut fallback will be used): {e}")
    ...
    # Optional: check diffusion dependencies
    try:
        import torch
        ...
        try:
            ...
            print(f"⚠ Diffusers not installed (diffusion refinement disabled)")
    except ImportError:
        print(f"⚠ PyTorch not installed (diffusion refinement disabled)")
...
def check_config():
    ...
    try:
        ...
        print(f"  - Diffusion model: {settings.diffusion_model_id}")
        print(f"  - Diffusion device: {settings.diffusion_device}")
