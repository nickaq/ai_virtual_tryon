...
## Запуск у Docker (Сервер / Linux)

### Передумови
- **NVIDIA GPU** на сервері
- **Docker** та **Docker Compose**
- **NVIDIA Container Toolkit** ([інструкція зі встановлення](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html))

### Налаштування
1. Налаштуйте файл `.env` у корені проекту.
2. Запустіть сервіси:
```bash
docker-compose -f docker-compose.prod.yml up -d --build
```

### Можливості Docker-конфігурації
- **GPU Acceleration**: Автоматичне резервування NVIDIA GPU для бекенду.
- **Persistence**: Збереження результатів, логів та моделей у `storage/` та `models/`.
- **Next.js Standalone**: Оптимізована збірка фронтенду для продакшену.
- **Rewrites**: Автоматичне проксіювання API-запитів до бекенд-контейнера.
